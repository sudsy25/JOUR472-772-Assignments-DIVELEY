...
**A1. The months with the greatest percentage of calls were the later months, while the lowest were earlier in the year, Jan and Feb. Theres an upward trend as the year progresses, with a spike in July.**

```{r}
pg_calls_month <- pg_calls |>
  group_by(month) |>
  summarise(total_calls = n()) |>
  mutate(percent_total = total_calls / sum(total_calls) * 100)

pg_calls_month |>
  ggplot(aes(x = factor(month), y = percent_total)) +
  geom_col() +
  labs(
    title = "Overdose 911 Calls By Month in PG County",
    x = "Month",
...
**A2. The line graph shows a slow upward climb of total calls per week, with major spikes found in week 28 and 51, there was a dip in late spring.**
...
pg_calls_week |>
  ...
  geom_line() +
  labs(
    title = "Weekly 911 Overdose Calls in PG County",
    x = "Week of Year",
    y = "Total Calls",
...
#pt 3
pg_zip_joined <- left_join(pg_calls_zip, md_income, by = "zipcode")
```
