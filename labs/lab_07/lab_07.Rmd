...
# pt 5
joined_data <- md_18_up |>
  left_join(moco_totals, join_by(GEOID==zip)) |>
...
```{r}
new_joined_data <- joined_data |>
  mutate(
    population_min = population - moe,
    population_max = population + moe,
    rate_min = (total_calls / population_max) * 1000,
    rate_max = (total_calls / population_min) * 1000
  )
```
