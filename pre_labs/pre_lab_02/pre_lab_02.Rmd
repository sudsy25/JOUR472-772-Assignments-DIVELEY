...
**Answer: When we want to edit these numbers, we don't want to override the original dataset. We may want to store both sets of information.**
...
**Answer: OR added all classes that have \> 15 seats, and added all theater classes that may have \<15 seats.**
...
**Answer: It turned / into -, changing the format of the date. It's now in ymd form.**
...
**Answer: R adds a floor date, so all the months expenses is represented by the floor date, aka the first of its month.**
...
**Answer: R is summing up all the votes that came in, and then seeing what percent of those votes were for Moore.**
...
**Answer: The data is now sorted by the highest percentage because we asked. it to arrange by the highest percent descending. We can now see the county that had the highest percent of their votes being for Moore.**
...
**Answer: I mean they both have their unique uses, as I like to see both the highest percentage and the lowest percentage. Moore may want to know the lowest percentage to know where he needs to do more campaigning or something.**
...
**Answer: "**str_to_lower()" does the opposite and changes everything to lowercase. "str_to_title()" makes the first letter of each word capital and the rest lowercase.
...
**Answer: It first examines when a state is represented in the "address" column, populates a new column called "state" with the abbreviated version of the state if one is found. If not, it returns an "NA" by default.**
...
**Answer: This does make sense, it adds a cleaner column intead of attempting to group by the address column and when it says the state. It takes that extra step to help easily sort the data.**
...
1.  Which payee received the most spending, and in what month was the payee's peak?

2.  What department offered the most sections ins 2025? How does it compare to 2024?

3.  What percentage of classes were in-person vs online?
