...
**Task** Run the following code and describe the dataframe it outputs. **Answer: This dataset tells us the names of physicians and when they were punished and why they were punished.**
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer: it removes repeated entries, returning each item only once. It doesn't combine them though.**
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer: there is now only one column with all 1.3 million words used, all lowercase, no punctuation.**
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer: it now shows a bar graph of the top 25 most used words, and the x-axis tells us the frequency. It isn't that interesting, as the words on the chart are expected to me common like the, of, and to.**
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer: it now shows us the top 50 words used, after removing the common words. Also, it shows how many times it appears and the percentage of the remaining words it makes up. This is more interesting without the common words.**
...
**Task** Run the following code and describe the results. **Answer: now we can see the top ten non-stop words for each year. We can now compare the cost common meaningful words from each year's dataset, seeing if there are any differences or matches.**
...
**Task** Run the following code and describe the results. **Answer: This displays the top ten two word phrases before 2022. This can help us understand the most common reasons for disciplinary actions**
...
**Task** Run the following code and describe the results. **Answer: This displays the top ten two word phrases before 2024.**
...
**Task** Run the following code and describe the results. **Answer: now we see the top 10 past the first of 2024.**
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer: We can now see the mostly negative list of words and their sentiment. Patient can be negative in our case, same with appropriate and effective aren't really used in a positive way.**
