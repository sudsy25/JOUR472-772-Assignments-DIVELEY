...
**Answer: I would assume you are attempting to count how many classes are offered in each term.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: It's in descending order, from largest amount to least.**
...
**Answer: It is now first sorted by term, and then within the term it's sorted by class name alphabetically.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: He combined the terms into one value, counts the total classes, then combines all the seats and sorts them in descending order.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer: We found the mean and median values of the seats, and the mean_seats sort of follow descending order, but not exactly. The median_seats usually stays in that 20-25 range the whole way down.**
...
**Answer: Now we are sorting by department instead of term, meaning now you can see the amount of classes and seats per department. We also added min/max columns of seats. It's then sorted by the total seats in descending order.**
...
1.  Which department consistently offers the largest amount of classes per term?

2.  Which term has the highest class size?

3.  What departments show the biggest growth/declines in classes each term?
